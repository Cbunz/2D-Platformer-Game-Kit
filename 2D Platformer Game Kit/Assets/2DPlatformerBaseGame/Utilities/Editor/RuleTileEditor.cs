...
namespace UnityEditor
{
    ...
    internal class RuleTileEditor : Editor
    {
        ...
        public void OnEnable()
        {
            ...
            m_ReorderableList = new ReorderableList(tile.tilingRules, typeof(RuleTile.TilingRule), true, true, true, true);
            ...
            if (tile.tilingRules != null && tile.tilingRules.Count > 0)
            {
                switch (tile.tilingRules[index].output)
                {
                    case RuleTile.TilingRule.OutputSprite.Random:
                        return k_DefaultElementHeight + k_SingleLineHeight * (tile.tilingRules[index].sprites.Length + 2) + k_PaddingBetweenRules;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_ReorderableList != null && tile.tilingRules != null)
            ...
            for (int y = 0; y <= 2; y++)
            {
                for (int x = 0; x <= 2; x++)
                {
                    ...
                    if (x != 1 || y != 1)
                    {
                        ...
                        {
                            tilingRule.neighbors[index] = (RuleTile.TilingRule.Neighbor)(((int)tilingRule.neighbors[index] + 1) % 3);
                            GUI.changed = true;
                            Event.current.Use();
                        }
                    ...
                    else
                    {
                        ...
                        if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
                        {
                            tilingRule.autoTransform = (RuleTile.TilingRule.AutoTransform)(((int)tilingRule.autoTransform + 1) % 4);
                            GUI.changed = true;
                            Event.current.Use();
                        }
                    }
        ...
        private static void OnSelect(object userdata)
        {
            ...
            data.m_Rule.autoTransform = data.m_NewValue;
        }
        ...
        private static void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
        {
            ...
            tilingRule.autoTransform = (RuleTile.TilingRule.AutoTransform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.autoTransform);
            y += k_SingleLineHeight;
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Collider");
            tilingRule.colliderType = (Tile.ColliderType)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.colliderType);
            y += k_SingleLineHeight;
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Output");
            tilingRule.output = (RuleTile.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.output);
            y += k_SingleLineHeight;

            if (tilingRule.output == RuleTile.TilingRule.OutputSprite.Animation)
            {
                GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Speed");
                tilingRule.animationSpeed = EditorGUI.FloatField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.animationSpeed);
                y += k_SingleLineHeight;
            }
            if (tilingRule.output == RuleTile.TilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.perlinScale = EditorGUI.Slider(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.perlinScale, 0.001f, 0.999f);
                y += k_SingleLineHeight;
            }
            ...
            if (tilingRule.output != RuleTile.TilingRule.OutputSprite.Single)
            {
                ...
                EditorGUI.BeginChangeCheck();
                int newLength = EditorGUI.IntField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.sprites.Length);
                if (EditorGUI.EndChangeCheck())
